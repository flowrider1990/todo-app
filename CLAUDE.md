...
- [style.css](style.css) — plain CSS, no variables/nesting. State is expressed through classes JS toggles: `.todo-item.done`, `.empty-state.hidden`, `body.theme-dark`, `.due-badge.overdue` / `.due-badge.due-today` (and the same pair on `.due-hint`), `.due-clear.hidden`, `.due-chip.active`, `.emoji-option.selected`, `.emoji-trigger.has-emoji`, `#todo-input.invalid`, plus the native `:disabled` state on `.clear-completed` and `[aria-expanded]` on `.emoji-trigger`. `.todo-error` collapses via `:empty` rather than a toggled class, so JS only ever sets its `textContent`.
...
Duplicate names are blocked on creation against **both** done and un-done tasks (`findDuplicate`), with different wording for each — a completed match tells the user to rename or clear it, since "Clear completed" is the way out. When a name matches both a done and an un-done task, the un-done one wins the message as the more actionable block. Comparison is trimmed and case-insensitive. Renaming inside the detail dialog is **not** checked, so a duplicate can still be created that way.
